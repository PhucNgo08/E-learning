...
def create_module(
    ...
    if not course:
        print("⛔ [Từ chối] Giáo viên không có quyền tạo module cho khóa học này.")
    ...
    if existed:
        print(f"⚠️ Module số {module_number} đã tồn tại trong khóa học {course.course_name}.")
...
# ============================================================
# 🔍 Lấy thông tin Module (và kiểm tra quyền sở hữu)
# ============================================================
def get_module_owned_with_course(db: Session, teacher_id: str, module_id: str):
    ...
    if not m:
        ...
        return None, None
    c = _course_owned(db, teacher_id, m.course_id)
    if not c:
...
def update_module(
    ...
    course = _course_owned(db, teacher_id, m.course_id)
    if not course:
        print("⛔ [Từ chối] Không có quyền chỉnh sửa module này.")
        return None

    # Kiểm tra trùng thứ tự module (trừ chính nó)
    duplicate = (
        db.query(Module)
        .filter(
            Module.course_id == m.course_id,
    ...
    m.learning_objectives = learning_objectives.strip() if learning_objectives else None
    m.updated_at = datetime.utcnow()

    db.commit()
    print(f"✏️ [Cập nhật] Module '{m.title}' đã được chỉnh sửa.")
    return m.course_id
...
def delete_module(db: Session, teacher_id: str, module_id: str):
    ...
    if hasattr(m, "deleted_at"):
        ...
        print(f"🕒 [Xóa mềm] Module '{m.title}' đã được đánh dấu xóa.")
    else:
        db.delete(m)
        print(f"🗑️ [Xóa cứng] Module '{m.title}' đã bị xóa hoàn toàn.")
...
def publish_module(db: Session, teacher_id: str, module_id: str, publish: bool):
    ...
    m, c = get_module_owned_with_course(db, teacher_id, module_id)
    if not m:
        print("⚠️ [Xuất bản] Không tìm thấy module hoặc không có quyền.")
        return None
    m.is_published = 1 if publish else 0
    m.updated_at = datetime.utcnow()
    db.commit()
    state = "✅ [Xuất bản]" if publish else "🚫 [Gỡ đăng]"
    print(f"{state} Module '{m.title}' thuộc khóa học '{c.course_name}'.")
    return m
